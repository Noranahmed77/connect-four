...
def greedy_ai(game, player):
    """Greedy AI that only looks one move ahead and picks the move with immediate highest score"""
    ...
    for col in valid_moves:
        ...
        # Greedy only evaluates the current board state after its move
...
def minimax_ai(game, depth, maximizing_player, player):
    """Standard Minimax implementation without pruning""" 
    valid_moves = game.get_valid_moves()
    ...
    if maximizing_player:
        ...
        for col in valid_moves:
            ...
            # Recursive call explores all branches
...
def alphabeta_ai(game, depth, alpha, beta, maximizing_player, player, transposition_table=None):
    """Alpha-Beta with transposition table and aggressive pruning"""
    if transposition_table is None:
        transposition_table = {}

    # Generate a unique key for the game state
    board_key = tuple(tuple(row) for row in game.board)
    key = (board_key, player, depth, maximizing_player)
    ...
    if depth == 0 or game.is_winner(player) or game.is_winner(3 - player) or game.is_full():
        eval = evaluate_board(game, player)
        transposition_table[key] = (eval, None)
        return eval, None
    ...
    if maximizing_player:
        ...
        for col in valid_moves:
            ...
            temp_game.drop_piece(col)
            temp_game.switch_player()
            eval, _ = alphabeta_ai(temp_game, depth-1, alpha, beta, False, player, transposition_table)
            
            # Early cutoff if we find a winning move
            if eval == math.inf:
                transposition_table[key] = (eval, col)
                return eval, col
            ...
            if beta <= alpha:
                break  # Beta cutoff
        transposition_table[key] = (max_eval, best_col)
        return max_eval, best_col
    else:
        min_eval = math.inf
        best_col = None
        for col in valid_moves:
            ...
            temp_game.drop_piece(col)
            temp_game.switch_player()
            eval, _ = alphabeta_ai(temp_game, depth-1, alpha, beta, True, player, transposition_table)
            
            # Early cutoff if we find a losing move
            if eval == -math.inf:
                transposition_table[key] = (eval, col)
                return eval, col
            ...
            if beta <= alpha:
                break  # Alpha cutoff
        transposition_table[key] = (min_eval, best_col)
        return min_eval, best_col

def a_star_ai(game, player):
    """A* search that considers both path cost and heuristic to find optimal sequence"""
    opponent = 3 - player
    
    # Priority queue: (f_score, g_score, move_sequence)
    open_set = []
    closed_set = set()
    
    # Initial state
    initial_state = (evaluate_board(game, player), 0, [])
    heapq.heappush(open_set, (-initial_state[0], initial_state))
    
    while open_set:
        _, current = heapq.heappop(open_set)
        f_score, g_score, moves = current
        
        # Create a temp game to simulate the moves
        temp_game = game.copy()
        for move in moves:
            temp_game.drop_piece(move)
            temp_game.switch_player()
        
        # Check if we have a winner in the simulated path
        if temp_game.is_winner(player):
            return moves[0] if moves else greedy_ai(game, player)
        if temp_game.is_winner(opponent):
            continue
        
        # If we've reached our depth limit (g_score represents depth here)
        if g_score >= 3:
            return moves[0] if moves else greedy_ai(game, player)

        # Generate child states
        for col in temp_game.get_valid_moves():
            new_game = temp_game.copy()
            new_game.drop_piece(col)
            new_game.switch_player()
            
            # Convert board to tuple for hashing
            board_tuple = tuple(tuple(row) for row in new_game.board)
            if board_tuple in closed_set:
                continue
            closed_set.add(board_tuple)
            
            # Calculate new g_score (depth) and f_score (heuristic)
            new_g_score = g_score + 1
            new_f_score = evaluate_board(new_game, player) - new_g_score  # f = h - g
            
            new_moves = moves + [col]
            heapq.heappush(open_set, (-new_f_score, (new_f_score, new_g_score, new_moves)))
    
    # Fallback to greedy if A* doesn't find a path
    return greedy_ai(game, player)
...
class ConnectFourGUI:
    def __init__(self, master):
        ...
        self.ai_delay = 500
        self.cell_size = 100  # Increased cell size
        self.colors = {
    ...
    def start_ai_vs_human(self):
        self.ai_players[1] = self.ai_var.get()
        self.ai_players[2] = None
        self.ai_setup_frame.destroy()
        self.setup_board()
